...
RTOS_SRC    := tasks.c list.c queue.c port.c heap_4.c
RTOS_DIR    := ./lib/FreeRTOS
INCLUDE_DIR += $(RTOS_DIR)/include
INCLUDE_DIR += $(RTOS_DIR)/portable/GCC/ARM_CM3
INCLUDE_DIR += $(RTOS_DIR)/portable/MemMang

SRC         += $(RTOS_SRC)

VPATH       := $(SRC_DIR) $(RTOS_DIR) 
VPATH       += $(RTOS_DIR)/portable/GCC/ARM_CM3 
VPATH       += $(RTOS_DIR)/portable/MemMang
